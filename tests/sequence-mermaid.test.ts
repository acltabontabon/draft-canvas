...
describe('toMermaid', () => {
  ...
  it('collapses a lone carriage return in labels and the title, not only \\n', () => {
    const m = model({
      title: 'Saga\rNext',
      participants: [
        { id: 'P1', alias: 'A', label: 'A\rB', category: 'service', kind: 'participant', sourceNodeId: 'a' },
        { id: 'P2', alias: 'B', label: 'B', category: 'service', kind: 'participant', sourceNodeId: 'b' },
      ],
      elements: [
        { kind: 'message', order: 0, from: 'P1', to: 'P2', label: 'Go\rnow', interaction: 'sync', sourceFlowId: 'f1', sourceEdgeIds: [], sourceStepIds: [] },
      ],
    });
    expect(toMermaid(m)).not.toContain('\r');
  });
