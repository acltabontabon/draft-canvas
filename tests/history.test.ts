...
/*
 * Actions touch nothing on the canvas, which makes them the one kind of edit the store's two
 * "did anything actually change?" gates would discard without either of them looking broken:
 * `shallowEqualDocument` compares a fixed list of fields, and so does `HistoryStack`'s
 * `sameContent`. Both failures are silent, so both get a test.
 */
describe('actions take part in history like any other edit', () => {
  beforeEach(reset);

  it('captures, completes and clears through undo and redo', () => {
    const id = store.getState().captureAction('Confirm timeout @Kevin');
    expect(id).not.toBeNull();
    expect(store.getState().document.actions).toHaveLength(1);

    store.getState().setActionDone(id!, true);
    expect(store.getState().document.actions[0]!.done).toBe(true);

    store.getState().undo();
    expect(store.getState().document.actions[0]!.done).toBeUndefined();

    store.getState().undo();
    expect(store.getState().document.actions).toHaveLength(0);

    store.getState().redo();
    expect(store.getState().document.actions).toHaveLength(1);
  });

  it('brings back what "Clear completed" removed', () => {
    const id = store.getState().captureAction('Add monitoring')!;
    store.getState().setActionDone(id, true);
    store.getState().clearDoneActions();
    expect(store.getState().document.actions).toHaveLength(0);
  ...
  it('records nothing for an edit that changed nothing', () => {
    const id = store.getState().captureAction('Check DLQ retention')!;
    const entries = store.getState().history.past.length;
    store.getState().updateActionText(id, 'Check DLQ retention');
    store.getState().setActionDone(id, false);
    store.getState().removeAction('not-a-real-id');
    expect(store.getState().history.past).toHaveLength(entries);
  });

  it('coalesces a burst of typing into one undo step, and drops it if it nets out', () => {
    const id = store.getState().captureAction('Check')!;
    const entries = store.getState().history.past.length;
    store.getState().updateActionText(id, 'Check D');
    store.getState().updateActionText(id, 'Check DL');
    store.getState().updateActionText(id, 'Check DLQ');
    expect(store.getState().history.past).toHaveLength(entries + 1);

    store.getState().undo();
    expect(store.getState().document.actions[0]!.text).toBe('Check');
  });
});
