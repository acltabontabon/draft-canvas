...
<img width="100%" alt="A run through Draft Canvas in six chapters: on a blank canvas, pressing a letter to drop a service under the cursor, naming it Orders API, dragging a connector into empty space to pick a queue, accepting the suggested worker with Tab, and a close-up of the finished chain where each connector carries the relationship it inferred — publishes to, consumed by — reading in the direction the arrow points; clearing it and using the command palette to drop in a composed CQRS architecture; writing a note and dropping it onto a connector; presenting a flow step by step, with close-ups of the code sample, the dropped note threaded with the starter's own note, and the read store's note appearing on their steps; telling the canvas it shows Containers from the command palette's C4 view-level picker, looking inside the Write Model with Cmd+Down to a room the app has worked out is Components, drawing an order handler and its data store there, opening the depth map that shows the way back out to the Orders canvas, and coming back out with Cmd+Up; opening the Learn handbook beside the canvas, searching for dead-letter queues and watching the recipe play; the editor following a switch to a light system theme; and closing cards for local-first storage and for the Apache 2.0 source, VS Code and Docker Hub" src="docs/media/demo.gif">
