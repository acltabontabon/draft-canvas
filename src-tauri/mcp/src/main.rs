...
What you send decides how clear the result can be. Before a first create_diagram, read get_capabilities topics [\"readability\"] once. In short: elements in reading order, entry point first; the main path as a flow in layout.primaryFlow; an about on every note; boundaries only for real ones (never external systems grouped just to tidy them); past ~15 elements, detail goes in an element's inside view. Then read the receipt: legibility lists crossings and connectors that detour or cut through boundaries, and advisories name what to change, with the update_diagram ops that do it. Apply the ones that make sense in the same turn.\n\
