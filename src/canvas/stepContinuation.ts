 ...
 */
export function stepContinuation(editor: Pick<EditorStore, 'document' | 'selection'>, delta: 1 | -1): boolean {
  const ui = useUiStore.getState();
  if (!ui.continuationsEnabled || ui.quickConnect || ui.contextMenu || ui.openAttachmentDetail) return false;
  ...
  const pick = delta === 1 ? candidates[0] : candidates.at(-1);
