...
| `deliveryRole` | `"dead-letter"`? | `queue` nodes only. Marks a node generated by "Add DLQ" as visually subordinate (dashed outline, muted opacity, a single-envelope icon, and a "DLQ" caption overriding `queueKind`'s own). Purely descriptive, never a reference to another node — *which* queue it belongs to lives entirely on the connecting edge (`semantic: "deadLetters"`, see [Relationship model](#relationship-model)), so this field can never dangle. Set only by the "Add DLQ" command, never by the manual queue-kind picker. |
...
| `routeMode` | `"direct"` ? | Set only once the user has explicitly taken this connector's routing over. Absent means Smart Routing may bundle it into a shared fan-out/fan-in trunk (a purely visual artifact — see [`docs/ARCHITECTURE.md`](ARCHITECTURE.md)); `"direct"` routes it on its own. Independent of `routing`, which is line style. |
| `label` | string? | |
| `accent` | enum? | As for nodes. |
| `details` | object? | `{ language, code }` — expandable detail shown on selection and during its playback step. |
| `semantic` | enum? | `http` · `grpc` · `event` · `command` · `query` · `reads` · `writes` · `publishes` · `consumes` · `calls` · `dependsOn` · `fansOut` · `deliversTo` · `ingests` · `replicates` · `cdc` · `syncs` · `deadLetters`. Optional convenience only — fills in a default `label` when picked on a labelless edge (skipped for a pairing the relationship model flags as unusual, see below), never assigned automatically, never changes `accent`. |
...
| `hasResponse` | boolean? | Whether this connector draws a second, quieter reply line back to its source, independent of whether `response` has text. Absent on a fresh connection — including Service↔Service, where the return path is implied at the level an architecture diagram works at — and turned on per connector from its own editor. A file that already recorded `true` keeps it. |
...
| `deliveryAttempts` | number? | Only meaningful alongside `semantic: "deadLetters"` — how many delivery attempts occur before this edge's failure route is taken, e.g. `3`. Renders as "after 3 attempts" rather than a claim about the underlying mechanism (broker redelivery, a visibility timeout, consumer-side retry — Draft Canvas models architectural intent, not a specific broker's retry implementation), which is also why the field isn't called `retryAttempts`. |
...
`deadLetters` is deliberately **not** part of the capability matrix — a manual Queue→Queue
connection (e.g. ordinary re-queueing) stays unrestricted and uninferred, exactly as any pairing
with no matrix entry does. It's reachable only through the "Add DLQ" command, which sets `semantic`
and `kind` explicitly on a freshly generated node, or by hand through the edge inspector's full
relationship list. "Does this Queue have a DLQ" is likewise never a stored flag on the node — it's
answered by checking for an outgoing `deadLetters` edge, so deleting either the edge or the
generated node (`deliveryRole: "dead-letter"`) cleanly reverts the relationship with no separate
cleanup step.
